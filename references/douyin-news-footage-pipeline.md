...
9. 先判断有效动态素材是否足以支持标准档；标准档预计覆盖至少 2 个场景类别，7–9 秒紧凑档至少覆盖 1 个直接相关类别。主播/AI 主播/对镜口播为零，单人脸部近景的预计占比不超过正文画面的 20%。
...
| 场景多样性与低人脸依赖 | 10% | 标准档至少 2 类；紧凑档至少 1 类直接相关场景；非主播且单人脸近景占比低 |
...
- 页面职责（事件建立、主体/动作、回应/结果等）和最终决定（使用/调整/拒绝）；视频素材的封面适用性恒为否，封面须另行检索独立高清图片。
...
主播、AI 主播和对镜口播片段不得进入最终时间轴。单个 `face_dominant=true` 的普通人物近景最长 1.5 秒，合计不超过正文画面的 20%；家庭、职场等人物场景应优先选择中景/全景、多人互动、动作和环境明确的画面。标准档至少覆盖 2 个 `scene_category`，推荐 3 类；7–9 秒紧凑档至少覆盖 1 个直接相关类别，并优先在节奏允许时覆盖第 2 类。
...
- [ ] 最终至少选定 1 条可追溯视频；标准档优先使用 1–3 条互补视频并覆盖至少 2 个场景类别，紧凑档至少覆盖 1 个直接相关类别。若加入图片/截图，已按 config 控制段数、占比并记录关键帧运动。
...
5. 禁止新闻主播、AI 主播、演播室或对镜口播进入最终时间轴或封面；普通人物人脸主导片段单段不超过 1.5 秒、合计不超过正文画面的 20%。标准档至少覆盖 2 个场景类别，紧凑档至少覆盖 1 个直接相关类别。禁止用 AI 生成画面替代真实视频或有来源的图片/信息截图。
...
3. **素材契约**：先核验和评分，再优先下载 1–3 条互补视频；区分直接证据与事实相关场景 B-roll，标准档至少覆盖 2 类场景，紧凑档至少覆盖 1 类直接相关场景。动态素材不足时可按 config 少量加入有来源的图片/信息截图与连续关键帧运动，并把主播、单一人脸、黑边和难清理文字作为显式风险。
