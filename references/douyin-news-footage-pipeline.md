...
2. 下载的是与新闻事件直接相关的真实现场、产品、设备、动作或结果画面。
...
若一条素材底部字幕可模糊且主体不受影响，可以继续使用。若模糊后仍不达标，则尝试其他时间段；仍不可用，再切换到第二或第三条已下载素材。
...
| # | Video ID | Canonical URL | Account | Local file | Duration | Resolution | SHA-256 | Selected reason |
|---|---|---|---|---|---:|---|---|---|
| 1 | `<id>` | `https://www.douyin.com/video/<id>` | `<账号>` | `素材/原片/douyin-<id>.mp4` | `<秒>` | `<宽×高>` | `<hash>` | `<真实现场/产品镜头等>` |
...
- `<video-id>`
  ...
  - Original text handling: `<裁切 / 底部局部模糊 / 不使用该时段>`
...
| 全程主播或演播室 | 搜索结果偏媒体口播 | 追加“现场/实拍/发布会/演示”等限定词，保留完整标题 | 把主播画面作为封面或核心 B-roll |
...
4. 按相关性、真实可用画面、字幕/黑边可处理性、稳定性和时长评分，只选 1–3 条。
5. 禁止选择新闻主播或演播室口播作为核心素材或封面；禁止用 AI 生成画面替代下载素材。
...
3. **素材契约**：先核验和评分，再下载 1–3 条互补素材；真实画面优先，主播、黑边和难清理文字是显式风险。
