# Changelog

## 1.14.2 — 2026-09-15

- 单视频发布流程规范化（管线 v1.3 第 9 节全文重写，依据 2026-09-15 折叠iPhone 实跑）：
  1. **干净会话入口**：浏览器有遗留草稿标签页（拆分视图/「商家平台」旧会话）时必须 `Ctrl+T` 新开标签页打开 publish_url；干净页标题「支付宝内容创作平台」+ 空白上传页，与旧「商家平台」草稿页区分；旧草稿原样保留并事后报告。
  2. **作者声明位置差异（关键）**：单页布局选项顺序为 内容为转载→内容含营销信息→内容虚构演绎→内容由AI生成→个人观点，仅供参考→**内容无需标注（最后一项）**，必须按名称选择；"选第一项"仅适用于批量编辑弹窗，单页第一项是「内容为转载」，点错会把作品标为转载。第 8 节已加交叉提示。
  3. **单条专属交互**：发布按钮为页底「确认发布」（左侧「保 存」勿点错），无批量确认弹窗、无进度环 N/N，成功直接跳「内容管理→已发布」；封面拦截处理复用第 7 节二次点击规则。
  4. **AX 战术**：radio 不暴露选中态，点击后以 focused 为落点信号；每次点击可能使无障碍状态过期，重新观察再点下一项；点「确认发布」前视觉复核三项实心选中（立即发布/公开/内容无需标注）与队列无进度标记。
  5. **保持默认不点击**：作品描述、话题与地点、征稿活动、加入合集、关联推荐、直播预告、AI智能优化、评论设置；消息封面设置区的「上传封面」为假入口禁止点击。
- config `platform_publish` 新增 `single_publish` 事实段（entry/declaration_option/publish_button/skip_fields/forbidden_clicks/ax_tactics）。

## 1.14.1 — 2026-09-15

- 依据 2026-09-15 单视频实跑（折叠iPhone，eSIM开卡指南）回写发布管线（v1.2）三条实战规则：
  1. **推荐缩略图高亮 ≠ 封面已设置**：点击「封面图片推荐」中与预览一致的缩略图后出现蓝色高亮，但封面字段可能仍未登记，「确认发布」会被「请选择视频封面」拦截；补救为重新点击该缩略图一次再发布（实测即通过），判据以确认发布不再报错为准。
  2. **「消息封面设置」假入口陷阱**：表单下方「消息封面设置」区自带「上传封面」按钮，点错会弹「消息封面」对话框；视频封面的上传入口在封面区推荐行第一格，点错时点「取 消」退出即可。
  3. **内容管理刚跳转可能显示「暂无作品」**：列表异步加载，等 3–5 秒再核验，以加载完成后的列表为准。
- config `cover_mode`、`verified` 同步更新。

## 1.14.0 — 2026-09-15

- 发布管线四大修复（依据 2026-09-15 用户反馈与页面截图，[references/alipay-publish-pipeline.md](references/alipay-publish-pipeline.md) 升至 1.1）：
  1. **Chrome 冷启动**：Chrome 未运行时 `open_application` 按名称经常找不到，改为完整路径 `C:\Program Files\Google\Chrome\Application\chrome.exe` 以 `--start-maximized --new-window` + 发布页 URL 冷启动，启动后等待并确认加载；禁止 `--user-data-dir`/`--incognito`（会产生无凭据干净实例）。已在运行时附着并最大化。
  2. **固定坐标快速连击**：进入发布页先锁定窗口状态并拍基准截图标定坐标，左队列槽位、封面推荐缩略图、批量编辑/发布等稳定控件按固定坐标连续点击（间隔 0.3–1 秒），不逐次截图；只在登录后账号校验、每条就绪门、每条封面应用、发布进度 N/N、内容管理核对五个校验点重新观察；任何校验失败或意外弹窗立即退回单步观察-点击模式，窗口状态改变则坐标作废重标。
  3. **登录后生活号校验与切换**：登录成功后平台停留在“上次使用的应用”，即使 URL 带 appId 也可能落在双V会员、冲鸭攒话费等其他生活号；以页面左上角账号卡名称为唯一判据，非目标时点账号卡「切」入口（或处理自动弹出的）「请选择应用」弹窗，按名称+ID 选中「手机通讯生活」并确定，复核左上角账号名生效后才继续上传。
  4. **封面推荐帧优先与队列点击切换**：多条目设置封面改为点击左中部视频队列缩略图槽位切换条目（禁用滚轮）；封面优先点击「封面图片推荐」中与预览大图一致的缩略图（零弹窗即设），替代旧「占位框→截取封面→完成」弹窗流程；推荐帧无匹配时才点「上传封面」走原生对话框粘贴主题目录 `封面.png`；每条应用后以预览大图更新为校验点。
- config `platform_publish` 新增 `browser_launch`、`account_switch`、`fast_click`、`item_switch` 事实键，`cover_mode` 更新为推荐帧优先+上传兜底，`verified` 追加 2026-09-15 修订记录。

## 1.13.0 — 2026-09-14

- 新增平台发布管线：用户验收成片后以“内容没问题，请帮我发布”“发布到手机通讯生活”等指令明确授权时，把输出区最终 MP4 发布到支付宝内容创作平台（手机通讯生活）。新增 [references/alipay-publish-pipeline.md](references/alipay-publish-pipeline.md) 执行手册与 config `platform_publish` 事实段（发布入口 URL、appId、账号、封面取首帧、声明策略、上传对话框与核验规则）。
- 发布环境硬性约束：必须用 computer-use 驱动用户日常 Chrome 真实配置文件，禁止独立自动化浏览器实例。理由：登录凭据只在用户 Chrome 密码管理器，干净实例无法登录且无短信通道；蚂蚁系风控对自动化指纹敏感；上传走 Windows 原生文件对话框属于网页+系统混合流程。
- 沉淀实跑教训为硬门：批量发布前必须逐条确认缩略图上传进度 100% 并消失，否则平台静默丢弃该条（无报错、不出现在任何状态标签，只能重传）；封面改为占位框 → 截取封面默认首帧 → 完成，不再单独上传封面图片文件（省一次原生文件对话框）；发布成败以内容管理已发布列表逐条核对为准。起因：2026-09-14 双视频批量发布中一条因未就绪被静默丢弃后重传补发。

## 1.12.0 — 2026-09-14

- 顶部标题区禁止出现新闻来源：主标题与黄色副标题条只承载新闻内容短语，来源固定以 18px `#717171` 小字渲染在每页正文板右下角 `source_safe_bbox`。时间轴顶层新增必填 `source_label`（如“界面新闻/财联社”），`production_contract.load_timeline` 校验 `headline`/`subtitle` 中不得包含其中的任何媒体名；G4/G7 增加对应视觉否决项。起因：2026-09-13/09-14 多条成片把“封面新闻·澎湃新闻”“潮新闻·澎湃新闻·川观新闻”写进了副标题条，正确样式参照“鸿蒙智行八月交付”样片右下角“界面新闻/财联社”。
- 图片/信息截图关键帧运动强制多倍超采样：新增 config `video.still_media.motion_supersample=4`，渲染器在 zoompan 前先按整数倍 lanczos 超采样再输出到目标尺寸，消除整像素采样导致的逐帧卡顿；render 报告记录 `still_motion_supersample`，最终机器验收校验该字段不低于 config 值。起因：“油价少涨了”样片中截图缩放动画明显卡顿。
- 最终 MP4 文件名改为与完整封面标题逐字一致：`cover_title_full` = 封面主标题 + 全角逗号 + 封面副标题（config `output.cover_title_full_pattern`）；`publish_news_output.ps1` 新增必填 `-CoverSubtitle` 并与 acceptance.json 的 `cover_title_full` 核对，`check_output_layout.ps1` 校验文件名以主题目录短名开头。主题目录仍为 `N.` + 封面主标题短名。起因：“1.渔民奇迹生还”成片只以主标题命名，应为“渔民奇迹生还，失踪11天回家.mp4”。

## 1.11.3 — 2026-09-09

- 用一张完整工作流信息图替换 README 顶部重复画面的接触表，使项目入口先说明流程与交付结果。
- 新增“一夜换季”“叫停畸形吃播”“最好的休息”三条 1080×1920 完整成片演示，封面可直接跳转到对应视频。
- 从三条成片的不同时间点各抽取两张正文截图，组成天气、社会治理、生活方式三类主题的真实画面矩阵，避免重复使用同一帧。
- 展示资产集中存放于 `assets/showcase/`，与 Skill 运行时参考资产、新闻生产输出和临时文件保持隔离。

## 1.11.2 — 2026-09-09

- 重构 GitHub 首页简介：补齐项目定位、完整功能矩阵、生产架构、标准流程、质量标准、安装调用、文件边界、文档导航与开发门禁。
- 使用仓库内已维护的接触表、封面与正文分板图组成图文演示，并提供可点击的完整参考视频入口；不新增来源不明或未经验证的展示素材。
- 明确单主题与多 Agent 批量模式、版本同步状态、发布目录边界和能力排除项，让其他 Agent 与首次使用者可以从 README 直接理解项目。

## 1.11.1 — 2026-09-08

- 封面候选新增四项人工编辑硬门：技术清晰、主体可辨、主题高度相关、视觉冲击；任一单项未达配置门槛即淘汰，不允许以总分或标题锐度补偿。
- 新增无标题 3:4 底图、270×360 无标题缩略图和候选接触表的前置验收；必须在不依赖封面文字的情况下看清新闻主体、动作或场景，再进入标题排版。
- 明确视觉冲击来自真实主体、关键动作、尺度、透视、光色或情绪张力，不得用无关人脸、夸张暗化、过饱和、虚假拼贴或耸动误导替代；无合格候选时返回重新搜图并阻塞封面定稿。

## 1.11.0 — 2026-09-08

- 新增根 Agent + 分阶段子智能体编排：事实锁定后并行文案节奏、视频素材和封面素材，时间轴保持单一写入者，锁定后再并行声音与画面，最终由独立 QA 只读验收、根 Agent 唯一发布。
- 版本门新增父运行清单与 `CHILD_CONTEXT_READY` 子模式。每个用户请求仍由根 Agent 严格联网核对 GitHub；子智能体只校验父清单 SHA-256、有效期、固定 commit、干净工作树和核心契约哈希，不重复联网或沿用旧清单。
- 新增可机器验证的任务包/交接包契约、角色独占输出目录、输入输出 SHA-256、最多五条关键决定和跨目录写入拦截，长日志与媒体证据留在工作区，减少聊天上下文回传。
- 修正最终视觉门的场景数量冲突：标准档至少两类场景，7–9 秒紧凑档至少一类直接相关场景；正文合格静态素材允许铺满实拍区，但仍受段数、占比和关键帧运动限制。

## 1.10.1 — 2026-09-07

- 修正主执行顺序、素材评分表与最终契约中残留的“所有成片至少两类场景”歧义：标准档至少两类，7–9 秒紧凑档至少一类直接相关场景，并优先在节奏允许时覆盖第二类。
- 在主执行步骤补齐图片/信息截图的来源、哈希、支持事实与关键帧记录，确保紧凑档不会因旧清单被迫凑长或误判失败。

## 1.10.0 — 2026-09-07

- 封面来源升级为强制独立图片：禁止候选视频帧、播放器截图、短视频缩略图和成片反抽帧；新增来源方法、原页面、用途依据、图片哈希、`derived_from_video=false`、清洁复核和有效像素契约，渲染器直接读取图片。
- 新增标准档与 7–9 秒紧凑档。真实动态素材不足时先精简为单一信息任务并使用紧凑档，不再重复、慢放、冻结或用无效镜头硬凑 14 秒。
- 正文允许少量可追溯高清图片或事实信息截图，默认最多 2 段且不超过正文 30%；必须绑定支持事实并使用连续、克制的缩放/平移关键帧。信息截图还须完成可见文字与 claim 映射，真实视频仍须占主要时长。
- 固定渲染器新增图片/截图输入和确定性 Ken Burns 运动；时间轴契约拒绝无关键帧静态图、跳动/超限运动、静态占满正文、来源缺失或封面视频帧。
- 保留现有三段式版式与素材检索规范，增加固定图文渲染适配器和唯一时间轴契约；测量字形可见边界，超宽文案直接拦截，不自动缩字。
- 按用户确认的内容驱动分页规则：每页信息任务、阅读最低停留、阅读依据与换页理由成为必需字段；配音和阅读共同约束页长，页面节奏与镜头切点独立。清除近似均分与自动延长示例；增加手机尺寸静音阅读、带声播放的发布门。
- 拦截阅读停留不足、跨页、截尾、页号错误、旧稿配音、音频调包和无说明重复源区间，不通过缩字或明显加速赶稿。
- MiniMax 完整参数从配置读取；新增页号/报告、文本/参数/WAV哈希绑定及实测缓存。旧命令需增加 --page-id；新混音需 --timeline，固定延迟仅留显式 --legacy 历史对照。
- 新混音逐页实测处理后的人声/BGM差，修复原最终响度目标用错、峰值未纳入PASS与迭代trim错记；合成保留视频流并记录音画到MP4的来源链。
- 最终QA拒绝空期望，检查逐帧PTS连续、完整帧数、编码、音频与来源哈希；抽帧批量处理，分板复用同哈希QA结果，减少重复全片扫描。
- 发布新增必需 -AcceptanceReport，拒绝内部诊断/旧证据/未完成视觉听觉事实验收的文件。机器检查不宣称 FINAL_READY。
- 真实本地回归验证：两页MiniMax、原始下载素材、固定版式编码、混音和最终QA；坏封面被拦截，样片未发布。具体耗时仅属于该样例，不是全流程速度承诺。

## 1.9.2 — 2026-09-05

- 根据平台“封面模糊或拉伸”告警增量强化封面质量门；保持已确认的版式、字号、配色、默认 9:16 母版与两文件交付，不从截图推断平台必须使用其他比例。
- 清除活动规范中六处仍允许封面局部模糊的冲突条款；局部字幕模糊例外仅适用于正文。
- 增加原生无缩放抽帧、裁切后的有效像素、等比缩放与封面专用放大阈值；新增独立几何校验脚本和回归测试，几何通过不代表清晰度通过。
- 区分无损封面母版、平台上传文件、缩略图和编码后首帧验收证据；只有明确的目标比例要求才另作等比裁切，不放大缩略图、不拉伸母版。
- 新增 100% 无文字底图检查、上传告警恢复与留证规则；已知未解决的平台告警阻止交付，未执行平台上传则如实标记未验证，不承诺推荐效果。

## 1.9.1 — 2026-09-05

- 修正版本门的远程名称约束：运行脚本现在直接读取并强制校验 `config.json` 中的 `skill_update.remote`，所有远程读取与快进获取使用同一个受信任名称，避免配置与实际执行路径漂移。
- 更新后必须用同一 run_id 再执行一次新版版本门，以最终 LATEST_READY 放行；明确核心文件检查与发布前完整一致性检查的区别，并补上环境示例的失败退出判断。

## 1.9.0 — 2026-09-04

- 新增每次调用都必须先执行的 GitHub 最新版本启动门：实时比较远端 `main` 与活动安装的 40 位 commit SHA，同 SHA 快速通过，远端领先时只做干净工作树上的 fast-forward 更新。
- 新增 `scripts/ensure_latest_skill.ps1`：固定可信仓库与分支、禁用交互式凭据提示、限制网络超时与重试、用跨进程互斥锁避免多 Agent 同时更新，并输出带 `run_id` 的结构化版本门 JSON。
- 版本门采用严格失败策略：断网、本地改动、错误远程、分支不符、历史分叉、远端核验变化或更新后核心文件校验失败时进入 `BLOCKED_SKILL_VERSION`，不得静默使用旧版继续。
- 明确一次运行只在启动时核验一次并固定 commit；发生更新后必须重新读取最新版 Skill，N1 再把状态、时间、版本和 SHA 写入 `run-manifest.json`。

## 1.8.0 — 2026-09-04

- 新增仓库根 `config.json` 作为全部制作 FACT（输出路径、音色与模型、混音参数、成片规格、坐标锁定文件引用）的唯一机器事实源；修改任何值只改一处。
- 文档全面瘦身去复述：SKILL.md、profile-v2、README 与 7 个 reference 中的输出路径、坐标数字、音色 ID、混音响度数值全部改为引用 config 或锁定 JSON；坐标的唯一人读镜像限定在 `locked-layout-validation.md`，音色 API 说明限定在 `minimax-tts.md`。
- 脚本直接消费 config：`minimax_tts.py` 的默认模型/音色、`publish_news_output.ps1` 与 `check_output_layout.ps1` 的输出根目录（`-OutputRoot` 变为可选参数）均从 config 读取，忘传/传错路径的空间归零。
- 新增 `scripts/mix_news_audio.py` 自动混音：按 config 把各页人声归一到目标段响度、BGM 自动计算 12dB 避让增益、限幅后实测综合响度并有限迭代校准；输出结构化指标 JSON。回归验证与 2026-09-04 手工版逐项一致（I -15.1 LUFS / TP -3.5 / 逐页差 11.9，且一次迭代命中）。
- 一致性门禁升级：新增 config 完整性检查（必需段、锁定文件与 BGM 资产存在性）与"文档复述 FACT"检查（输出路径/坐标模式/音色 ID 只允许出现在 config、CHANGELOG 与两份镜像文档）。

## 1.7.0 — 2026-09-04

- 规范去双轨化：删除 SKILL.md 的 V1 条款与 `10a/15a` 补丁覆盖结构，封面零模糊与 `layout-lock-v2.json` 坐标直接并入正文条款；V1 历史仅留 CHANGELOG。
- 全量清理 V1 残留：`locked-layout-validation`、`cover-platform-layout-v2`、`editorial-sop`、`quality-standards`、`delivery-gates`、`visual-audio-template` 中的 V1 分板与文字坐标全部更新为 V2；`visual-audio-template` 的配音节从 Edge TTS 规范重写为 MiniMax 定版规范；`environment-setup` 移除 Edge TTS 安装与冒烟步骤。
- TTS 脚本合并：`minimax_tts.py` 直接暴露 `--voice/--speed/--vol/--pitch/--emotion/--timbre`，删除 `minimax_tts_pro.py` 分身；新增 `voice_source` 输出字段（`explicit/env/default_fallback`），音色静默回退从此可见。
- 新增 `scripts/check_skill_consistency.py` 一致性门禁（禁用术语、V1 坐标、内部链接、资产引用、Python 语法、锁定文件状态），配 `hooks/pre-commit` 以 `core.hooksPath` 分发；首次运行即清除 11 FAIL / 29 WARN 的规范漂移。

## 1.6.0 — 2026-09-04

- 默认音色定版为 `Chinese (Mandarin)_News_Anchor`（用户五音色试听对比确认），`Reliable_Executive` 降为备用男声；两个 TTS 脚本默认值同步。
- 新增 `minimax_tts_pro.py`：暴露 speed/vol/pitch/emotion/timbre_weights 参数，并记录发音词典、停顿标记、账户音色查询接口（`POST /v1/get_voice`）等扩展能力。
- 写入实测混音定式：人声约 +2.4dB、BGM-01 0–14s 平切 -9.4dB、`alimiter=0.668`，并记录人声增益超过约 +4dB 触发限幅器"天花板泵"的陷阱。
- 发布流程整体改为输出流程：输出区从 `<项目根>/outputs/` 迁移到独立磁盘路径 `D:\每日新闻\YYYY-MM-DD\N.中文新闻短名\`，工作区仍为项目根 `.news-editor-work`；全部文档与目录检查提示同步更新。

## 1.5.0 — 2026-09-04

- 新增最高优先级 V2 制作配置：扩大中段实拍区，并按最新平台参考重排、下移正文文字组；机器坐标源切换为 `layout-lock-v2.json`。
- 封面改为严格使用清晰、干净、无字幕、无马赛克和无模糊的原始干净帧；正文保留仅限底部字幕的局部模糊例外。
- 增加素材有效裁切分辨率、放大倍率、单次高质量编码和强制人工清晰度检查。
- 默认配音迁移到 MiniMax 官方 T2A，同步加入凭据停点、安全环境变量、受限重试、逐页 WAV 和人工听检；真正调用 API 前才向用户请求本机配置。
- 最终 MP4 强制与封面主标题同名，移除“新闻女声”等变体后缀，并在目录检查中验证名称一致。

## 1.4.0 — 2026-09-02

- 从用户确认的 14 秒参考成片抽取封面、两页稳定帧、分页切点、末帧以及标题板、实拍板、正文板独立参考资产。
- 新增 `layout-lock-v1.json`，把封面、标题条、1054px 实拍区、白字、红字、来源和底线的像素坐标设为唯一坐标源。
- 新增 `extract_layout_proof.py`，对草稿与最终 MP4 自动抽取封面、缩略图、各页三块分板、切点和末帧，并输出结构化证据清单。
- 将全部视觉规范统一到确认成片实测布局，同时保留用户后续确认的深海蓝底板；板块边界偏差超过 2px 或未逐板查看时不得交付。

## 1.3.0 — 2026-09-01

- 将封面与正文底板统一为深海蓝渐变，新增透明 V3 四角框资产，禁止纯黑空封面和旧版不透明暗层遮住真实画面。
- 将正文实拍中段放大到 `1080×938`，正文固定为 1–2 行白色说明加 1 行红色重点，并定义平台 Logo 的可控遮挡阈值。
- 当精确标题结果全是主播或人脸近景时，要求从事实包推导职场、家庭、办事流程等场景关键词，并把素材区分为直接证据与场景 B-roll。
- 增加主播/AI 主播/对镜口播零容忍、普通人脸单段 1.5 秒与总占比 20%、至少 2 类场景等素材与时间轴硬门。
- 扩展素材清单、剪辑计划和 QA 报告字段，确保其他 Agent 必须提交场景类别、人脸占比、封面真实画面与红字遮挡证据。

## 1.2.0 — 2026-09-01

- 在原 N0–N11 流程内补齐节点状态、证据文件、失败回退和定向修改的复验要求。
- 细化抖音候选页确认、下载后完整解码、目标裁切首中末帧、有效时码与多素材互补剪辑标准。
- 封面改为先换帧与重构图、只对实际文字框精确局部模糊，禁止用大块模糊牺牲有效信息。
- 正文恢复用户确认的原版三段式布局，固定黄线、副标题、实拍层和上移后的平台安全文字区。
- 细化双页语义、逐页试音、帧数分配、人声/BGM stem 测量、最终抽帧与发布哈希验收。

## 1.1.0 — 2026-08-28

- 将固定盘符、用户名、旧版 `outputs/today-news.../project` 路径改为运行时项目变量。
- 统一使用 `<ProjectRoot>/outputs` 发布区与 `<ProjectRoot>/.news-editor-work` 内部工作区。
- 浏览器 profile、Cookie、下载元数据、截图和原片不再进入发布区。
- 增加稳定发布与目录检查脚本，强制日期、连续序号和中文主题命名。
- 更新封面字号、平台安全区、素材裁字、底部字幕模糊和最终验收规范。

## 1.0.0 — 2026-08-24

- 建立热点发现、事实核验、抖音精准检索、真实素材剪辑、新闻女声、BGM、封面与最终 QA 工作流。

