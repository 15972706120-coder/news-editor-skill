...
每个新用户请求先由根 Agent 对实际加载的 Skill 根目录运行联网版本门；同一请求内被编排的子智能体按 [子智能体编排规范](subagent-orchestration.md) 使用父清单做本地证明，不重复联网。新电脑第一次使用、工具升级后或完整制作开始前，再运行环境预检。不得从多个目录静默选择“第一份”而掩盖版本漂移：
